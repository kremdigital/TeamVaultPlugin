...
describe('DocManager — purgeUnknownBindings', () => {
  it('deletes stores of bindings the caller does not know, keeps the rest', async () => {
    // The state.db-less case: nothing names `b9`, but its databases are right
    // there in the enumeration.
    const idb = makeFakeIdb([
      'team-vault-b1-keep.md',
      'team-vault-b9-gone.md',
      'team-vault-b9-also-gone.md',
    ]);
    const dm = new DocManager({ idb: idb.registry });

    const deleted = await dm.purgeUnknownBindings(['b1']);

    expect(deleted.sort()).toEqual(['team-vault-b9-also-gone.md', 'team-vault-b9-gone.md']);
    expect([...idb.names]).toEqual(['team-vault-b1-keep.md']);
  });

  it('leaves other plugins’ databases alone', async () => {
    const idb = makeFakeIdb(['team-vault-b9-gone.md', 'some-other-plugin-db', 'team-vaultish']);
    const dm = new DocManager({ idb: idb.registry });

    await dm.purgeUnknownBindings([]);

    expect([...idb.names].sort()).toEqual(['some-other-plugin-db', 'team-vaultish']);
  });

  it('does not confuse binding ids that share a prefix', async () => {
    const idb = makeFakeIdb(['team-vault-b1-x.md', 'team-vault-b10-y.md']);
    const dm = new DocManager({ idb: idb.registry });

    await dm.purgeUnknownBindings(['b1']);

    expect([...idb.names]).toEqual(['team-vault-b1-x.md']);
  });

  it('closes cached docs of unknown bindings before deleting (open handles block)', async () => {
    const factory: PersistenceFactory = (name, doc) => new FakePersistence(name, doc);
    const idb = makeFakeIdb(['team-vault-b9-a.md']);
    const dm = new DocManager({ persistenceFactory: factory, idb: idb.registry });
    dm.get('b9', 'a.md');
    const [a] = FakePersistence.instances;

    const deleted = await dm.purgeUnknownBindings(['b1']);

    // clearData (close + delete), not destroy (close) — a still-open handle
    // makes `indexedDB.deleteDatabase` hang forever instead of failing.
    expect(a?.cleared).toBe(true);
    expect(dm.has('b9', 'a.md')).toBe(false);
    expect(deleted).toEqual(['team-vault-b9-a.md']);
    // The name is reported once, not deleted twice: the enumeration pass
    // skips what the cache pass already handled. (`FakePersistence.clearData`
    // only flips a flag — real y-indexeddb drops the db from the registry.)
    expect(idb.deleted).toEqual([]);
  });

  it('is best-effort — a failed delete does not throw', async () => {
    const idb: IdbRegistry = {
      list: () => Promise.resolve(['team-vault-b9-a.md']),
      delete: () => Promise.reject(new Error('blocked')),
    };
    const dm = new DocManager({ idb });

    await expect(dm.purgeUnknownBindings(['b1'])).resolves.toEqual([]);
  });
});
